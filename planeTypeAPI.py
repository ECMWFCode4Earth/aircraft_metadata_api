...
def load_tzutc():
    ...
    with open("./rawdata/timezone/tz.txt") as fp:
        for line in fp:
            ...
            print(tmp)
            session.execute(
                "insert into Timezone (timezone, utcdiff) VALUES( '%s' , '%s' )"
                % (tmp[0], tmp[2])
            )
    session.commit()
    session.close()
...
def convertTimeZone(datestr, _time, timezone):
    ...
    if timezone[0].isalpha():
        try:
            session = session_factory()
            local_tz = session.execute(
                f"select utcdiff from Timezone where timezone = '{timezone}'"
            )
        except:
            session.close()
            return None
...
def diffdistance(long1, lat1, long2, lat2):
    ...
    a = math.sin(latdiff / 2) * math.sin(latdiff / 2) + math.cos(
        ph1
    ) * math.cos(ph2) * math.sin(longdiff / 2) * math.sin(longdiff / 2)
...
class flightawareAPI:
    ...
    def SearchBirdseyePositions(self, latitiude, longtitude):
        ...
        payload = {
            "query": f"{{range lat {latitiude-2} {latitiude+2}}}  {{range lon {longtitude -2} {longtitude + 2}}}  }}",
            "howMany": "15",
            "uniqueFlights": "true",
            "offset": "0",
        }
        response = requests.get(
            self.fxmlUrl + "SearchBirdseyePositions",
            params=payload,
...
class api:
    def __init__(
        self,
        chrome_path=os.getcwd() + "/chromedriver/chromedriver",
        username=None,
        apiKey=None,
    ):
        ...
        self.driver = webdriver.Chrome(
            options=chrome_options, executable_path=self.chrome_driver
        )
    ...
    def getcountry_latlon(self, latitiude, longtitude, buffer=0):
        ...
        response = requests.get(fxmlUrl, params=payload)
    ...
    def get_airline_fleet(self, airline):
        ...
        time.sleep(s)
        self.driver.get(
            f"https://www.flightradar24.com/data/airlines/{airline}/fleet"
        )
        click_down = self.driver.find_elements_by_css_selector(
            'i[class="pull-right fa fa-angle-down"]'
        )
        for x in click_down:
            x.click()
        # self.driver.execute_script("arguments[0].setAttribute('style','display: block;')", drop)
        table = self.driver.find_elements_by_css_selector(
            'table[class="table table-condensed table-hover"]'
        )
        res = []
        for x in table:
            type_code = x.find_element_by_xpath("..")
            type_code = self.driver.execute_script(
                "return arguments[0].previousElementSibling", type_code
            )
            ...
            for r in rows:
                ...
                res.append(
                    [
                        td[0].find_element_by_tag_name("a").text,
                        td[1].text,
                        type_code,
                    ]
                )
    ...
    def _getTypeByID(self, flightID, epochtime, option=0):
        ...
        if option == 0:
            if flightID[2].isalpha():
                aline = self.session.execute(
                    f"select iata from airline where icao = '{flightID[:3]}'"
                ).fetchone()
                if not aline:
                    return None
            ...
            self.driver.get(
                "https://www.flightradar24.com/data/flights/" + flightID
            )
            ...
            try:
                ...
                return "del"
            datarow = self.driver.find_elements_by_css_selector(
                'tr[class=" data-row"]'
            )
            for i in range(len(datarow)):
                ...
                for ep in epochtime:
                    if toepoch(ep) >= std and toepoch(ep) <= sta:
                        ...
                        print(
                            f"time {toepoch(ep)} not between deptime {std} and arrtime {sta}"
                        )
        elif option == 1:
            try:
                self.driver.get(
                    "https://flightaware.com/live/flight/%s" % flightID
                )
                table = self.driver.find_element_by_css_selector(
                    'div[id="flightPageActivityLog"]'
                )
                table = table.find_elements_by_css_selector(
            ...
            while t < 3:
                try:
                    if len(table) == 2:
                        first = table[0].find_element_by_css_selector(
                            'div[class="flightPageDataRowTall flightPageDataRowActive"]'
                        )
                        ptype = first.find_elements_by_css_selector(
                            'div[class="flightPageActivityLogData optional"]'
                        )[0].text
                        date = first.find_elements_by_css_selector(
            ...
            for x in range(len(table)):
                ...
                while t < 3:
                    try:
                        rows = table[x].find_elements_by_css_selector(
            ...
            while t < 3:
                try:
                    ...
                    if len(table) == 2:
                        first = table[1].find_element_by_css_selector(
                            'div[class="flightPageDataRowTall flightPageDataRowActive"]'
                        )
                        tmptime = first.find_elements_by_css_selector(
            ...
            for x in range(len(table)):
                ...
                for i in range(len(rows)):
                    ...
                    while t < 3:
                        try:
                            tmptime = rows[i].find_elements_by_css_selector(
                                'div[class="flightPageActivityLogData"]'
                            )
                    ...
                    for arrt in arrtime:
                        for ep in epochtime:
                            ...
                            if edept < earrt:
                                if (
                                    toepoch(ep) >= edept
                                    and toepoch(ep) <= earrt
                                ):
                                    dep = (
                                        row[2].split("-")[-1].replace(" ", "")
                                    )
                                    arr = (
                                        row[3].split("-")[-1].replace(" ", "")
                                    )
                                    return [row[0], dep, arr]
                                else:
                                    print(
                                        f"time {toepoch(ep)} not between deptime {edept} and arrtime {earrt}"
                                    )
        return None
    ...
    def get_tailnumber(self, tailnumber, options=0):
        ...
        if options == 0:
            try:
                self.driver.get(
                    f"https://www.flightradar24.com/data/aircraft/{tailnumber}"
                )
                table = self.driver.find_element_by_css_selector(
                    'table[id="tbl-datatable"]'
                )
                data_row = table.find_elements_by_css_selector(
                    'tr[class=" data-row"]'
                )
                type_code = self.driver.find_element_by_css_selector(
                    'div[id="cnt-aircraft-info"]'
                )
                type_code = type_code.find_element_by_css_selector(
                    'div[class="col-xs-7"]'
                )
                type_code = type_code.find_element_by_css_selector(
                    'div[class="row h-30 p-l-20 p-t-5"]'
                )
            ...
            for x in range(len(table)):
                ...
                for row in range(len(rows)):
                    try:
                        tmp = []
                        data = rows[row].find_elements_by_css_selector(
                            'div[class="flightPageActivityLogData optional"]'
                        )
                        date = rows[row].find_elements_by_css_selector(
                            'div[class="flightPageActivityLogData flightPageActivityLogDate"]'
                        )
                        times = rows[row].find_elements_by_css_selector(
                            'div[class="flightPageActivityLogData"]'
                        )
    ...
    def get_airport(
        self, lat1, long1, range=4, international=False, distance_range=250
    ):
        ...
        cur = self.session.execute(
            "select icao, latitude, longitude from Airport where longitude  between %f and %f and latitude between %f and %f %s"
            % (arange[0], arange[1], arange[2], arange[3], inter)
        )
        res = []
        for row in cur:
    ...
    def distance_diff_airport(
        self, airport1, airport2, code1="icao", code2="icao"
    ):
        airport1 = self.session.execute(
            f"select latitude,longitude from Airport where {code1} = '{airport1}'"
        ).fetchone()
        airport2 = self.session.execute(
            f"select latitude,longitude from Airport where {code2} = '{airport2}'"
        ).fetchone()
    ...
    def diffdistance_one_airport(self, lat1, long1, airport, code="iata"):
        airport1 = self.session.execute(
            f"select latitude,longitude from Airport where {code} = '{airport}'"
        ).fetchone()
    ...
    def get_international_airport_wiki(self):
        ...
        table2 = self.driver.find_elements_by_css_selector(
    ...
    def getRoutebyPort(self, dep, arr):
        res = self.session.execute(
            "select flightid from route where arr='%s' and dep = '%s'"
            % (arr, dep)
        )
    ...
    def getRoutebyAware(self, dep, arr):  # ICAO
        self.driver.get(
            "https://flightaware.com/live/findflight?origin=%s&destination=%s"
            % (dep, arr)
        )
        datarow = self.driver.find_elements_by_css_selector(
            'td[class="ffinder-results-ident text_align_left"]'
        )
        routes = set()
        for row in datarow:
    ...
    def getRoutebyStat(
        self, dep, arr, _date
    ):  # for hour 0 - 0-6, 6 - 6-12, 12 -12-18, 18 - 0
        if type(_date) != str:
            _date = str(_date)
        year = int(_date[:4])
        month = int(_date[4:6])
        ...
        for x in hours:
            _path = (
                "https://www.flightstats.com/v2/flight-tracker/route/%s/%s/?year=%s&month=%s&date=%s&hour=%s"
                % (dep, arr, year, month, day, x)
            )
            self.driver.get(_path)
            try:
                self.wait.until(
                    lambda driver: self.driver.find_element_by_css_selector(
                        'div[class="table__Table-s1x7nv9w-6 iiiADv"]'
                    ).is_displayed()
                )
            except:
                continue
            try:
                ...
                datarow = table.find_elements_by_css_selector(
                    'div[class="table__TableRowWrapper-s1x7nv9w-9 ggDItd"]'
                )
            except:
                continue
            for row in datarow:
                route = row.find_element_by_css_selector(
                    'h2[class="table__CellText-s1x7nv9w-15 KlAnq"]'
                ).text
        ...
        with open("./rawdata/routes.tsv") as fp:
            ...
            for line in fp:
                ...
                if len(tmp[2]) > 2 and len(tmp[4]) > 2:
                    session.execute(
                        "insert into Route (flightid, dep,arr) VALUES( '%s' , '%s' , '%s' )"
                        % (
                            tmp[0],
                            tmp[2].replace("'", ""),
                            tmp[4].replace("'", ""),
                    ...
                    if count % 1000 == 0:
                        try:
                            session.commit()
                            print(
                                "successfully inserted 1000 row, total inserted: %d"
                                % count
                            )
                        except:
                            print("error")
...
class airportdb:
    ...
    def loadlonghaul(self):
        ...
        for x in airports:
            if len(x) == 3:
                session.execute(
                    "UPDATE Airport SET international = 1 where iata = '%s'"
                    % (x)
                )
        session.commit()
        a.close()
...
class planetypedb:
                        ...
                        if row[3] not in result[amdarids]:
                            for flights in position_data[amdarids]:
                                if (
                                    ...
                                    continue
                                head_tail = (
                                    flights[0:1] + flights[len(flights) - 1 :]
                                )
                                for x in head_tail:
                                    current_time = toepoch(x[3])
                                    print(
                                        f"testing time {current_time} is between {f[2]} and {f[3]}"
                                    )
                                    if (
                                        f[2] <= current_time
                                        and f[3] >= current_time
                                    ):
                                        print(current_time, f[0], f[1])
                                        print(x[0], x[1])
                                        dist_from_dep = self.api.diffdistance_one_airport(
                                            x[0], x[1], f[0], "iata"
                                        )
                                        dist_from_arr = self.api.diffdistance_one_airport(
                                            x[0], x[1], f[1], "iata"
                                        )
                                        print(dist_from_arr, dist_from_dep)
                                        if (
                                            dist_from_arr > 1000
                                            and dist_from_dep > 1000
                                        ):
                                            print(f[4])
                                            result[amdarids][row[3]] = False
                                            print(
                                                f"{row[3]} is false for {amdarids}"
                                            )
                                            break
                                else:
                                    continue
    ...
    def loaddata(
        self,
        international=True,
        lower_distance_diff=4000,
        upper_distance_diff=20000,
        predict_step=0,
        time_diff=3600,
        auto_predict=False,
        airport_search_dist=250,
        no_estimate=True,
    ):
        filelist = os.listdir("./rawdata/amdw")
        filelist.sort(key=lambda x: int(x.split(".")[1]))
        ...
        with open("./statistic/airportMatchResult.txt", "a") as statairport:
            for bi in range(len(filelist)):
                ...
                numair = 0
                statairport.write(
                    f"airport matching statistic for file {file} \n"
                )
                # get airport
                with open("./rawdata/amdw/" + file) as fp:
                    for line in fp:
            ...
            if predict_step:
                for amdarid in position_data:
                    for flight in position_data[amdarid]:
                        ...
                        forward_direction = get_directions(
                            flight[len(flight) - 5 :]
                        )
                        for x in range(predict_step):
                            ...
                            predict_for_lat = current_forward_test[0] + (
                                forward_direction[0]
                            )
                            predict_for_lon = current_forward_test[1] + (
                                forward_direction[1]
                            )
                            dd = diffdistance(
                                predict_lon,
                            ...
                            dd_for = diffdistance(
                                predict_for_lon,
                                predict_for_lat,
                                current_forward_test[1],
                                current_forward_test[0],
                            )
                            predict_time = datetime.strptime(
                                current_test[3], "%Y%m%d%H%M%S"
                            ) + timedelta(minutes=int(dd / 500))
                            predict_alt = current_test[2] - 100
                            predict_time_for = datetime.strptime(
                                current_forward_test[3], "%Y%m%d%H%M%S"
                            ) + timedelta(minutes=int(dd_for / 500))
                            ...
                            flight.append(
                                [
                                    ...
                                    predict_for_lon,
                                    predict_for_alt,
                                    predict_time_for.strftime("%Y%m%d%H%M%S"),
                                ]
                            )
                            flight.insert(
                                0,
                                [
                                    ...
                                    predict_lon,
                                    predict_alt,
                                    predict_time.strftime("%Y%m%d%H%M%S"),
                                ],
                            )
            ...
            for amdarid in position_data:
                ...
                for flight in position_data[amdarid]:
                    ...
                    print(head_tail)
                    for x in head_tail:
                        match = self.api.get_airport(
                            x[0],
                            x[1],
                            international=international,
                            distance_range=airport_search_dist,
                        )
                        ...
                        if auto_predict:
                            if len(flight) >= 5:
                                ...
                                backward_direction = get_directions(
                                    list(reversed(flight[:5]))
                                )
                            while not match and tried > 0:
                                ...
                                predict_lat = current_test[0] + (
                                    direction[0] / 2
                                )
                                predict_lon = current_test[1] + (
                                    direction[1] / 2
                                )
                                dd = diffdistance(
                                    predict_lon,
            ...
            statairport.write(
                f"total line : {lwa} , total number of both airport matched: {numair} \n"
            )
            for i in list(flightIDs):
                for flight in position_data[flightIDs]:
                    ptype = self.api._getTypeByID(
                        i, [i[3] for i in flight], option=1
                    )
                    if ptype:
                        self.session.execute(
                            "insert into Planetype ( amdarid, flightid,planetype) VALUES( '%s' , '%s' , '%s')"
                            % (i, i, ptype)
                        )
                    ...
                    for dep in range(len(val)):
                        for arr in range(dep + 1, len(val)):
                            ...
                            if i + str(k) in tried_routes:
                                if [deport, arrport] in tried_routes[
                                    i + str(k)
                                ]:
                                    continue
                            dist_diff = self.api.distance_diff_airport(
                                arrport, deport
                            )
                            if (
                                dist_diff < lower_distance_diff
                                or dist_diff > upper_distance_diff
                            ):
                                print(
                                    f"distance between 2 airport is less than {lower_distance_diff}"
                                )
                                continue
                            print(
                                f"{i} searching route between {deport} and {arrport}"
                            )
                            b = self.get_route(
                                deport, arrport, position_data[i][k][0][3]
                            )
                                ...
                                if not tried_b and not b:
                                    ...
                                    if len(position_data[i][k]) >= 5:
                                        ...
                                        for direct in range(len(directions)):
                                            print(
                                                f"direct index {direct} , len of direction {len(directions)}"
                                            )
                                            next_airport = None
                                            predict_lon = 0
                                            while (
                                                ...
                                                predict_lat = current_test[
                                                    0
                                                ] + (directions[direct][0] / 2)
                                                print(predict_lon)
                                                predict_lon = current_test[
                                                    1
                                                ] + (directions[direct][1] / 2)
                                                print(predict_lon)
                                                ...
                                                next_airport = self.api.get_airport(
                                                    predict_lat,
                                                    predict_lon,
                                                    international=international,
                                                )
                                            ...
                                            for j in next_airport:
                                                if direct == 1:
                                                    print(
                                                        f"{i} searching route between {j} and predicted airport {arrport}"
                                                    )
                                                    b = self.get_route(
                                                        j,
                                                        arrport,
                                                        _date=predict_time.strftime(
                                                            "%Y%m%d%H%M%S"
                                                        ),
                                                    )
                                                    if b:
                                                        ...
                                                        print(
                                                            f"added predicted departural airport {j}"
                                                        )
                                                else:
                                                    print(
                                                        f"{i} searching route between {deport} and predicted airport {j}"
                                                    )
                                                    b = self.get_route(
                                                        deport,
                                                        j,
                                                    ...
                                                    if b:
                                                        ...
                                                        print(
                                                            f"added predicted arrival airport {j}"
                                                        )
                                                if b:
                                                    break
                                            else:
                                    ...
                                    if i + str(k) not in tried_routes:
                                        tried_routes[i + str(k)] = [
                                            [deport, arrport]
                                        ]
                                    else:
                                        tried_routes[i + str(k)].append(
                                            [deport, arrport]
                                        )
                            for x in b:
                                print("testing for %s" % x)
                                planetype = self.api._getTypeByID(
                                    x,
                                    [i[3] for i in position_data[i][k]],
                                    option=0,
                                )
                                ...
                                if planetype and planetype != "del":
                                    ...
                                    if source == "flightaware":
                                        if (
                                            self.api.distance_diff_airport(
                                                arr1, arrport, "iata", "icao"
                                            )
                                            > 100
                                            or self.api.distance_diff_airport(
                                                dep1, deport, "iata", "icao"
                                            )
                                            > 100
                                            or self.api.distance_diff_airport(
                                                arr1, dep1
                                            ...
                                            print(
                                                f"distance between 2 airport is more than {100}km"
                                            )
                                            break
                                    matchedType += 1
                                    self.session.execute(
                                        "insert into Planetype ( amdarid, flightid,planetype, dep,arr,datasource,time) VALUES( '%s' , '%s', '%s', '%s', '%s', '%s', '%s')"
                                        % (
                                            i,
                                            x,
                                            planetype,
                                            ...
                                            arr1,
                                            source,
                                            position_data[i][k][0][3],
                                        )
                                    )
                                    if matchedType % 5 == 0:
                                        self.session.commit()
                                    print(
                                        "\n planetype %s matched for %s and %s"
                                        % (planetype, x, i)
                                    )
                            else:
    ...
    def get_route(self, dep, arr, _date):
        ...
        record = self.session.execute(
            f"select * from noroute where dep='{dep}' and arr = '{arr}'"
        )
        if record.fetchone():
            return []

        b = self.api.getRoutebyStat(
            dep, arr, _date
        )  # + self.api.getRoutebyAware(dep,arr)
        if not b:
            self.session.execute(
                f"insert into noroute(dep, arr) VALUES ('{dep}', '{arr}')"
            )
            self.session.commit()
            return []
        for x in b:
            indb = self.session.execute(
                "select * from Route where flightid = '%s'" % x
            ).fetchone()
            if not indb:
                self.session.execute(
                    "insert into Route ( flightid, dep,arr) VALUES( '%s' , '%s' , '%s')"
                ...
                with open("./rawdata/amdw/" + file, "w") as fout:
                    for x in data:
                        ...
                        try:
                            ...
                            continue
                stat.write(
                    f"unfiltered record for this file is {unfiltered} \n"
                )
                stat.write(
                    f"after filter by altitude below {alt}, number of records is {filtered}\n"
                )
    ...
    def writePlanetypedate(self, day=0, amdarid=None):
        ...
        f = open(
            f"all_aircrafttype_{str(today).replace('-','')}_{str(lastweek).replace('-','')}.txt",
            "a",
        )
        res = self.session.execute("select * from planetype")
        f.write(
            "amdar    flightid  planetype time                   dep    arr    datasource \n"
        )
        for row in res:
            if amdarid:
                if row[1] not in amdarid:
                    continue
            f.write(
                f"{row[1]}  {row[2]}   {row[3]}  {row[4]}     {row[5]}     {row[7]}       {row[9]}    \n"
            )
        f.close()
    ...
    def writeAirline_fleet(self, airline):
        for x in airline:
            ...
            f.write(
                "tailnumber    type-code     airline_iata        airline_icao        type_description \n"
            )
            ...
            if len(air_code) == 2:
                ...
                code = "icao"
            icao = self.session.execute(
                f"select iata, icao from Airline where {code} = '{air_code.upper()}'"
            ).fetchone()
            if not icao:
                icao = []
            ...
            for row in res:
                f.write(
                    f"{row[0]}          {row[2]}              {icao[1]}             {icao[0]}                    {row[1]} \n"
                )
        f.close()

    def writePlanetyperesults(
        self, day=0, count=2, maximum=False, amdarid=None, validate=True
    ):
        today = date.today()
        dict1 = {}
        lastweek = today - timedelta(days=day)
        f = open(
            f"multiple_aircrafttype_{str(today).replace('-','')}_{str(lastweek).replace('-','')}.txt",
            "a",
        )
        res = self.session.execute("select * from planetype")
        ...
        for keys in dict1:
            ...
            for planetype in k:
                if validate:
                    ...
                    if (
                        tmp_list.count(planetype) >= count
                        and v_validate[keys][planetype]
                    ):
                        f.write(
                            f" {keys}         {planetype}     {tmp_list.count(planetype)}  \n"
                        )
                else:
                    print(planetype)
    ...
    def write_tailnumber(self, tailnumber, airline=None, options=0):
        ...
        if airline:
            f = open(
                f"airline_{airline}_{str(today).replace('-','')}_validate.txt",
                "a",
            )
        elif "-" not in tailnumber:
            f = open(
                f"airline_{tailnumber[0][-2:]}_{str(today).replace('-','')}_validate.txt",
                "a",
            )
        else:
            f = open(
                f"airline_{tailnumber[0].split('-')[0]}_{str(today).replace('-','')}_validate.txt",
                "a",
            )
        for x in tailnumber:
            ...
            for flight in data:
                if options == 0:
                    f.write(
                        f"{flight[4]}      {flight[0]}        { epochToUtc(flight[2])}   {flight[1]}     { epochToUtc(flight[3])}   \n"
                    )
                else:
                    f.write(
                        f"Unknown                 {flight[0]}        { epochToUtc(flight[2])}   {flight[1]}     { epochToUtc(flight[3])}   \n"
                    )
        f.close()
    ...
    def loaddata_statistic(self, amdarids, alt_filter):
        ...
        for x in amdarids:
            f.write(
                f"{x}          {dict2[x]}                        {dictfilter[x]} \n"
            )

        dict1 = {i: {} for i in amdarids}
        f.write("\n ************************ airport *********** \n")
        f.write(
            "amdar    flightid  planetype  dep    depcount  arr  arrcount  datasource \n"
        )
        apis = api()
    ...
    def loaddata(self):
        ...
        with open("./rawdata/airlines.dat") as fp:
            for line in fp:
                ...
                if tmp[7] == "Y":
                    session.execute(
                        f"insert into Airline (iata, icao, name) values ('{tmp[3]}', '{tmp[4]}', '{tmp[1]}')"
                    )
                    count += 1
        try:
            session.commit()
